  ...
  const onSubmit = (data: any) => {
    ...
    const payload = {
      ...
      items: selectedItems.map((item) => ({
        ...
        productVariantId: item.selectedVariant ? item.selectedVariant.id : undefined,
    ...
    createOrder(payload, {
      onSuccess: (response: any) => {
        ...
        fbEvent("Purchase", {
          ...
          content_name: selectedItems.map((item) => item.name),
          content_ids: selectedItems.map((item) => item.slug || String(item.id)),
